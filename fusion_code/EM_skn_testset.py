...
side_name = 'past'
tol = '5e-4'
train_dir = f'./EM_sknorm/sknorm_norm/train'
test_dir = f'./EM_sknorm/sknorm_norm/test'
...
for ws in range(8,9):
    ...
    #         a1 = pd.read_excel(r1_file)['Rating'].to_numpy()
    #         a2 = pd.read_excel(r2_file)['Rating'].to_numpy()
    #         a4 = pd.read_excel(r4_file)['Rating'].to_numpy()
    #         a5 = pd.read_excel(r5_file)['Rating'].to_numpy()
            
    #         # data_dict[subject_id]['R1'] = np.hstack((tq, np.diff(a1)))
    #         # data_dict[subject_id]['R2'] = np.hstack((tq, np.diff(a2)))
    #         # data_dict[subject_id]['R4'] = np.hstack((tq, np.diff(a4)))
    #         # data_dict[subject_id]['R5'] = np.hstack((tq, np.diff(a5)))
            
    #         data_dict[subject_id]['R1'] = a1
    #         data_dict[subject_id]['R2'] = a2
    #         data_dict[subject_id]['R4'] = a4
    #         data_dict[subject_id]['R5'] = a5
