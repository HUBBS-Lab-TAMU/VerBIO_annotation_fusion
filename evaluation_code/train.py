...
def train(task, model, train_loader, epoch, optimizer, criterion, use_gpu=False):
    ...
    for batch, batch_data in enumerate(train_loader, 1):
        features, feature_lens, labels = batch_data
...
def train_model(task, model, data_loader, epochs, lr, current_seed, use_gpu, criterion,
                regularization=0.0):
    ...
    best_model_file = ''
    early_stop = 0
    res_dict = {'Train Loss': [], 'Val Loss':[], 'CCC': []}
    for epoch in range(1, epochs + 1):
        ...
        res_dict['Val Loss'].append(val_loss)
        res_dict['CCC'].append(val_score)
    ...
    return best_val_loss, best_val_score, best_model_file, res_dict
